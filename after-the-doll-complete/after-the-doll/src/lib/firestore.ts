...
// Archive functions
export async function createArchive(archive: Omit<Archive, 'archiveId'>): Promise<string> {
  const docRef = await addDoc(collection(db, 'archives'), {
    ...archive,
    createdAt: Timestamp.fromDate(archive.createdAt),
    updatedAt: Timestamp.fromDate(archive.updatedAt),
  });
  await updateDoc(docRef, { archiveId: docRef.id });
  return docRef.id;
}
...
export async function getArchivesByUser(uid: string): Promise<Archive[]> {
  const q = query(
...
export async function updateArchive(archiveId: string, updates: Partial<Archive>): Promise<void> {
...
export async function getEntriesByArchive(archiveId: string): Promise<JournalEntry[]> {
  const q = query(
    collection(db, 'entries'),
